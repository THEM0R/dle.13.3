		...
		$rss_option = <<<HTML
			<div class="row mt-15">
				<div class="col-sm-6" style="max-width:300px;">
					...
					<div class="checkbox"><label><input class="icheck" type="checkbox" name="allow_rss_turbo" value="1" {$ifrsst}>{$lang['allow_rss_turbo']}</label></div>
				</div>
				<div class="col-sm-6">
					<div class="checkbox"><label><input class="icheck" type="checkbox" name="allow_rss_dzen" value="1" {$ifrssd}>{$lang['allow_rss_dzen']}</label></div>
				</div>
			</div>
HTML;
	...
	if( $item_db[0] ) {
		...
		if( $have_perm ) {
			...
			if( $ifdelete != "yes" ) {
				...
				if ($item_db[6]) $db->query( "UPDATE " . PREFIX . "_post_extras SET related_ids='', allow_rate='$allow_rating', votes='$add_vote', disable_index='$disable_index', access='$group_regel', editdate='$added_time', editor='{$member_id['name']}', reason='$editreason', view_edit='$view_edit', disable_search='{$disable_search}', need_pass='{$need_pass}', allow_rss='{$allow_rss}', allow_rss_turbo='{$allow_rss_turbo}', allow_rss_dzen='{$allow_rss_dzen}' WHERE news_id='$item_db[0]'" );
				else $db->query( "INSERT INTO " . PREFIX . "_post_extras (news_id, allow_rate, votes, disable_index, access, editdate, editor, reason, view_edit, disable_search, need_pass, allow_rss, allow_rss_turbo, allow_rss_dzen) VALUES('{$item_db[0]}', '{$allow_rating}', '{$add_vote}', '{$disable_index}', '{$group_regel}', '{$added_time}', '{$member_id['name']}', '{$editreason}', '{$view_edit}', '{$disable_search}', '{$need_pass}', '{$allow_rss}', '{$allow_rss_turbo}', '{$allow_rss_dzen}')" );
